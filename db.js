...
const getPost = (id) => {
  return db.query('SELECT p.*, l.*, c.* FROM posts p LEFT JOIN LATERAL (SELECT json_agg(l) as likes FROM likes l WHERE l.like_post_id = p.post_id) l ON TRUE LEFT JOIN LATERAL (SELECT json_agg(c) AS comments FROM comments c WHERE c.comment_post_id = p.post_id) c on TRUE WHERE p.post_id = $1', [id])
}
...
const deletePost = (post_id, user_id) => {
  return db.query('DELETE FROM posts WHERE post_id = $1 AND post_user_id = $2 RETURNING *', [post_id, user_id])
}

const checkLike = (post_id, user_id) => {
  return db.query('SELECT * FROM likes WHERE like_post_id = $1 AND like_user_id = $2', [post_id, user_id])
}

const createLike = (post_id, user_id) => {
  return db.query('INSERT INTO likes (like_post_id, like_user_id) VALUES ($1, $2) RETURNING *', [post_id, user_id])
}

const deleteLike = (post_id, user_id) => {
  return db.query('DELETE FROM likes WHERE like_post_id = $1 and like_user_id = $2 RETURNING *', [post_id, user_id])
}

const createComment = (user_id, post_id, text) => {
  return db.query('INSERT INTO comments(comment_user_id, comment_post_id, comment_text) VALUES ($1, $2, $3) RETURNING *', [user_id, post_id, text])
}
...
const deleteComment = (comment_id, user_id) => {
  return db.query('DELETE FROM comments WHERE comment_id = $1 AND comment_user_id = $2 RETURNING *', [comment_id, user_id])
}
